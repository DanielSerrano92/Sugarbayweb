...
import { serializeVideoFilters } from "@/lib/media/filters";
import type { MediaVideoFilters } from "@/lib/media/types";
...
function buildHref(basePath: string, filters: MediaVideoFilters, page: number): string {
  const params = serializeVideoFilters(filters, { page });
...
export default function VideoPagination({
  ...
  return (
    <nav aria-label="Paginacion de videos" className="mt-6 flex flex-wrap items-center gap-2">
      <Link
